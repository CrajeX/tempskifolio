...
const NotificationPanel = () => {
  ...
  // Calculate unread notifications count for each tab
  const unreadCounts = {
    news: notifications.news.filter(notif => notif.status === "unread").length,
    application: notifications.application.filter(notif => notif.status === "unread").length
  };
          ...
          <div className="notifications-tabs">
            {Object.keys(notifications).map((tab) => (
              ...
              >
                {tab.charAt(0).toUpperCase() + tab.slice(1)}
                {unreadCounts[tab] > 0 && (
                  <span className="unread-badge">{unreadCounts[tab]}</span>
                )}
              </button>
            ))}
          </div>
